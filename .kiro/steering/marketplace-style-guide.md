...
When working with marketplace pages and components, always follow these style rules:

## Colors

### Primary Colors
- `ink` (#101618) - Main color for text, borders, dark backgrounds
- `primary` (#00bfff) - Primary accent color, buttons, links
- `accent-lime` (#ccff00) - Highlights, success states, active items
...
### Border Style
- Always use `border-2 border-ink` for main cards, buttons, inputs
- Use `border-gray-200` for secondary elements, inactive states
- Border radius: `rounded-lg` (default), `rounded-xl` for large cards, `rounded-full` for badges/pills
...
```
- Use `shadow-hard-sm` for buttons, small cards
- Use `shadow-hard` for cards, modals
- Use `shadow-hard-lg` for featured items, hero sections
