...
export default function DatasetDetailPage() {
  ...
  // Resolve SuiNS name for seller
  const { data: sellerSuiNSName, isLoading: isLoadingSuiNS } = useSuiNSName(
    listing?.seller,
  );
    ...
    if (processingMode && listing) {
      ...
      tl.to(overlayRef.current, { opacity: 1, duration: 0.2 })
        ...
        .add(() => {
          if (iconRef.current)
            iconRef.current.innerText =
              processingMode === "download" ? "lock_open" : "cloud_upload";
          if (textRef.current)
            textRef.current.innerText =
              processingMode === "download"
                ? "DECRYPTING SHARDS"
          ...
          if (subTextRef.current)
            subTextRef.current.innerText =
              processingMode === "download"
                ? "Reassembling Walrus Protocol data..."
        ...
        .add(() => {
          if (iconRef.current) {
            iconRef.current.innerText =
              processingMode === "download" ? "download" : "visibility";
    ...
    reviews: [
      {
        ...
        date: "2 days ago",
        comment:
          "Extremely high quality data. The JSON formatting is clean and required zero cleanup before ingestion into our analytics pipeline. Worth every SUI.",
        bgColor: "bg-blue-500",
      },
      {
        id: "r2",
        user: "AlphaLab Research",
        initials: "AL",
        rating: 4,
        date: "5 days ago",
        comment:
          "Solid dataset. The consumer behavior section is particularly detailed. Would recommend for DeFi analysis.",
        bgColor: "bg-primary",
      },
    ],
    features: [
    ...
    <div className="flex flex-col lg:flex-row h-full w-full overflow-hidden">
      ...
      <div className="w-full lg:w-[400px] shrink-0 bg-accent-lime border-b-2 lg:border-b-0 lg:border-r-2 border-ink flex flex-col p-6 lg:p-8 gap-6 overflow-y-auto z-10">
        ...
        <div className="flex flex-col gap-2 mt-4">
          ...
          <p className="text-sm font-bold text-ink opacity-80">
            Updated: {dummyData.updatedAt}
          </p>
        </div>
        ...
        <div className="mt-auto flex flex-col gap-4">
          <div className="flex flex-col">
            <span className="text-sm font-bold uppercase tracking-widest text-ink opacity-60">
              Current Price
            </span>
            <span className="text-5xl font-black tracking-tighter text-ink">
              {formatPrice(listing.price)}
            </span>
            <span className="text-sm font-bold text-ink opacity-60">
              ≈ ${((Number(listing.price) / 1e9) * 1.25).toFixed(2)} USD
            </span>
          </div>
          ...
          {isOwner ? (
            ...
            <button
              ...
              className="group w-full rounded-xl border-2 border-ink bg-ink py-4 text-white shadow-hard transition-all hover:-translate-y-1 hover:bg-primary hover:text-ink"
            >
            ...
            <button
              ...
              className="group w-full rounded-xl border-2 border-ink bg-ink py-4 text-white shadow-hard transition-all hover:-translate-y-1 hover:bg-primary hover:text-ink disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <div className="flex items-center justify-center gap-3">
                {isPurchasing ? (
                  <>
          ...
          {!account && !isOwner && (
            <p className="text-center text-xs font-bold text-ink opacity-60">
        ...
        <div className="max-w-5xl mx-auto p-6 lg:p-12 flex flex-col gap-12 relative z-10">
          ...
          <section>
            <div className="relative w-full aspect-[2/1] rounded-2xl border-2 border-ink overflow-hidden shadow-hard-lg group">
              {listing.imageUrl ? (
                ...
                <>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="material-symbols-outlined text-[120px] text-gray-600">
                      dataset
                    </span>
                  </div>
                </>
              ...
              <div className="absolute bottom-4 right-4 flex gap-2">
                ...
                <button
                  onClick={handleDownloadClick}
                  className="h-10 w-10 bg-white border-2 border-ink rounded-lg flex items-center justify-center hover:bg-accent-lime transition-colors shadow-hard-sm text-ink"
                >
          ...
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-3 p-4 bg-gray-50 rounded-xl border border-gray-200">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <code className="text-xs font-mono text-ink bg-white px-2 py-1 rounded border">
                    {listing.id.slice(0, 16)}...{listing.id.slice(-8)}
                  </code>
                  <a
                    href={`https://suiscan.xyz/testnet/object/${listing.id}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="h-6 w-6 rounded bg-white hover:bg-gray-100 flex items-center justify-center border"
                    title="SuiScan"
                  >
                    <svg
                    ...
                    >
                      ...
                      />
                      <path
                        d="M150 63.6C151.7 62.5 153.8 62.3 155.5 62.9C156.3 63.3 156.9 63.8 157.4 64.5C157.9 65.2 158.1 66.1 158 66.9L154.5 125C154 132.5 151.3 140.1 146.7 147.1C133.5 167.2 106.3 176.7 85.9 168.5C80.1 166.2 75.6 162.7 72.3 158.4C73.1 158.4 73.9 158.4 74.6 158.4C95.6 158.4 116.6 147.1 128.2 129.4C133.9 120.8 137.1 111.4 137.7 102.3L139.6 70L150 63.6Z"
                        fill="white"
                      />
                      <path
                        d="M123 40.3C124.7 39.2 126.8 39 128.5 39.6C129.2 39.9 129.9 40.5 130.3 41.2C130.8 41.9 131 42.7 130.9 43.5L127.4 101.6C127 109.1 124.2 116.7 119.6 123.7C106.4 143.8 79.2 153.3 58.8 145.1C38.5 136.9 32.7 114 45.9 93.9C50.5 86.9 57.1 80.7 64.6 76.1L123 40.3Z"
                        fill="white"
                      />
                    </svg>
                  </a>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <code className="text-xs font-mono text-ink bg-white px-2 py-1 rounded border">
                    {listing.blobId.slice(0, 16)}...{listing.blobId.slice(-8)}
                  </code>
                  <a
            ...
            <div className="flex flex-col gap-4">
              <div className="rounded-xl border-2 border-ink bg-gray-50 p-5 shadow-hard-sm">
                <h4 className="font-black text-lg mb-3 flex items-center gap-2 text-ink">
                ...
                <div className="flex flex-col gap-3">
                  {dummyData.versionHistory.map((ver, i) => (
                    <div
                      key={i}
                      className={`flex justify-between items-center text-sm ${!ver.isCurrent ? "opacity-60" : ""}`}
                    >
          ...
          <section className="flex flex-col gap-6 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {dummyData.reviews.map((review) => (
                <div
                  key={review.id}
                  className="rounded-xl border-2 border-ink bg-white p-6 shadow-hard-sm hover:shadow-hard transition-shadow"
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      <div
                        className={`h-10 w-10 rounded-full border-2 border-ink flex items-center justify-center font-bold text-white shadow-sm ${review.bgColor}`}
                      >
                        {review.initials}
                      </div>
                      <div className="flex flex-col">
                        ...
                        <div className="flex text-accent-orange">
                          {[...Array(5)].map((_, i) => (
                            <span
                              key={i}
                              className={`material-symbols-outlined text-[16px] ${i < review.rating ? "" : "text-gray-300"}`}
                            >
                              star
                            </span>
                          ))}
          ...
          <footer className="mt-10 border-t-2 border-ink pt-8 flex items-center justify-between opacity-60 pb-8">
            <p className="text-sm font-bold font-mono">
              ID: {listing.id.slice(0, 16)}...
            </p>
            <div className="flex gap-4">
              <span className="material-symbols-outlined cursor-pointer hover:opacity-100">
                flag
              </span>
              <span className="material-symbols-outlined cursor-pointer hover:opacity-100">
                share
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-white rounded-2xl border-2 border-ink shadow-hard-lg p-6 relative">
            <button
              ...
              className="absolute top-4 right-4 h-8 w-8 flex items-center justify-center rounded-lg hover:bg-gray-100"
            >
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                <h3 className="text-2xl font-black text-ink uppercase leading-none mb-2">
                  Confirm Purchase
                </h3>
                <p className="text-gray-600 font-medium">
                  You are about to purchase{" "}
                  <span className="font-bold text-ink">{listing.name}</span>.
                </p>
              </div>
              ...
              <div className="flex gap-3 mt-2">
                <button
                  onClick={() => setIsBuyModalOpen(false)}
                  className="flex-1 h-12 rounded-xl border-2 border-ink bg-white text-ink font-bold hover:bg-gray-100"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 h-12 rounded-xl border-2 border-ink bg-primary text-ink font-bold shadow-hard-sm hover:translate-y-0.5 hover:shadow-none transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {isPurchasing ? (
                    <>
      ...
      {processingMode && (
        <div
          ref={overlayRef}
          className="fixed inset-0 z-[60] flex flex-col items-center justify-center bg-ink/95 backdrop-blur-md opacity-0"
        >
          <div
            ref={sealRef}
          ...
          >
            <div
              className="absolute inset-2 rounded-full border-2 border-dashed border-white/30 animate-spin"
            ...
            <span
              ref={iconRef}
              className="material-symbols-outlined !text-[80px] text-white relative z-10"
            >
              lock
            </span>
            <svg
              ...
              >
                <textPath ref={pathRef} href="#circlePath2" startOffset="0%">
                  {processingMode === "preview"
                    ? "SECURE PREVIEW • REMOTE DECRYPTION •"
          ...
          <p
            ref={subTextRef}
            className="mt-2 text-sm font-bold text-gray-500 uppercase tracking-widest"
          >
